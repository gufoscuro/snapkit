...
This document describes the architecture for sharing state between sibling components in a configurable page system.
...
A working POC is available at `/poc/state-sharing`:

- **DemoFilter**: Provides `filters` (search text + status array)
- **DemoTable**: Consumes `filters`, provides `selection` (selected row IDs)

The filter and table communicate through the shared `filters` namespace without knowing about each other.
