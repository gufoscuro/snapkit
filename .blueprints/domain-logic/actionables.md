...
- **`RecordCustomerCell`** (shared, `features/common/`) — one identity column stacking a record **code** (linked, primary) over the **customer** name (muted subtitle). Used by *Da incassare* (`invoice.document_number` → `invoice-payments`, href guarded on the nested `invoice`), *Da fatturare* (`source.document_number` → `sales-order-details`/`transport-document-details` by source kind; customer via `extractSnapshotString(customer_snapshot,'name')`). Replicates the `link` renderer's anchor styling; like `LeftSidebarMenu` it renders `<a href>` without `resolve()` — the project's accepted pattern (`svelte/no-navigation-without-resolve` is knowingly tolerated here, same as the built-in link renderer which emits an unresolved href via a raw string).
...
**Where:** `RecordCustomerCell.svelte` (`features/common/`), `QuantityProgressCell.svelte` + `DeliveryDateCell.svelte` (`features/delivery-schedule/`) — all registry-scanned. Consumed by `PaymentsTable.svelte` + `InvoiceableDocumentsTable.svelte` (identity cell) and the two delivery tables (quantity/date cells). *Da spedire* no longer carries an identity **column**: since the backlog was grouped by source order, order number + customer moved into the group header (`TableGroupHeader`), so `DeliveryScheduleTable` dropped `RecordCustomerCell`. The per-order recap (`SalesOrderDeliveryScheduleTable`) can adopt the quantity/date cells for consistency but currently still lists them as separate columns (and needs no identity column — it's already scoped to one order).
...
## Da spedire — customer PO (ODA) as an opt-in column

**What:** `customer_purchase_order` is declared with `defaultVisible: false`, so it never shows until a user enables it from the column customizer. Same treatment on `SalesOrdersTable`.

**Why:** it's an **order-header** field, not line data — `GET /delivery-schedule` denormalizes it onto every line, so in the order-grouped backlog it repeats identically down each group. Combined with a table that is already the widest of the three actionables (see the condensed-cells section above), a permanently visible column would cost horizontal space for a repeated value. Opt-in keeps it one toggle away for whoever reconciles by customer PO. The field is optional free text on the order (`max:255` on `POST/PUT /sales-orders`); when unset the cell renders `-`.

Nothing prevents surfacing it in `TableGroupHeader` instead, where it would appear once per order — that's the natural home for an order-level reference, and the column is the deliberate but weaker choice.

**Where:** `DeliveryScheduleTable.svelte` and `SalesOrdersTable.svelte` — `customer_purchase_order` column. The endpoints that expose it are `GET /delivery-schedule` and `GET /sales-orders` (also `customer_purchase_order_date`, not surfaced); **invoices and transport documents do not carry it**, so the same column can't be added there without a backend change. The opt-in mechanism itself is documented in [resource-table.md](../components/resource-table.md#column-customization-reorder-hide-opt-in-columns).
