...
`width` replaces the default min-width class on the field's input/trigger (both fields concatenate it into their class list), so the field fills the dialog and can shrink. Real use: `RecordPaymentDialog`. On the input fields that carry a suffix it is also mirrored onto the field wrapper — see the next section.

## Fields With a Suffix: Size the Box, Style the Input

`NumberField`, `QuantityField`, `TextField` and `PriceField` render their suffix (`rightLabel`, or the currency symbol) as an element positioned `absolute right-0` **against the field wrapper**, not against the input. Two rules keep it honest.

### Rule 1 — width classes go to the wrapper, everything else stays on the input

```svelte
<div class="relative {fieldBoxClasses}">
  <Input class={classes} style={rightLabelStyle} … />
  {#if rightLabel}<div class="pointer-events-none absolute top-0 right-0 mr-2.5 …">{rightLabel}</div>{/if}
</div>
```

```typescript
// form.ts — keeps only w- / max-w- / min-w- / basis- / flex- tokens, variants included
...
**Why the wrapper needs the width:** sizing only the input leaves the wrapper full-width, so on a narrower field (`FormFieldClass.MaxWidth`) the suffix drifts to the right edge of the *column*, detached from the box.

**Why `className` still reaches the input, and only the width tokens are copied:** a caller's class can mix the two concerns. `FormFieldClass.TableCell` is exactly that — `h-10 w-full rounded-none border-transparent focus:border-primary focus-visible:ring-0` — where only `w-full` is sizing and the rest is what makes the input look like a table cell. Moving the whole class to the wrapper strips that styling off the input. Duplicating the width on both is harmless: the wrapper constrains, the input fills it.

`fieldBoxSizingClasses` lives in `form.ts` and is unit-tested in `form.test.ts` (note: that file runs in the **server** vitest project, not `unit`, which only picks up `*.unit.test.ts`).

### Rule 2 — reserve padding from the suffix's own length

```typescript
const rightLabelStyle = $derived(
  rightLabel ? `padding-right: calc(${rightLabel.length}ch + 1.25rem)` : undefined,
)
```

**Why:** a flat `pr-4` reserves the same room for `%` and for `CHF`, so a right-aligned number runs into the suffix. `ch` tracks the label's own characters, plus its `mr-2.5` margin and a small gap. It is an inline style rather than a class because a computed Tailwind class (`pr-[calc(…)]`) would never be generated by the JIT scanner.

**Don't** add `pr-8` / `pr-12` at the call site: four editors used to, as a workaround for both problems, and those were removed when the fields started reserving the space themselves.
