<script lang="ts">
  ...
  import EmailsFilters from '$lib/components/features/common/Filters/EmailsFilters.svelte'
  import { EmailsTable } from '$lib/components/features/emails/EmailsTable'
  ...
  // Set up default bindings: GenericFilters provides 'filters', EmailsTable consumes 'filters'
  setSnippetBindings({
...
<SettingsHeader legalEntityName={data.legalEntity?.name} breadcrumbs={[{ label: m.emails() }]} />

<div class="flex flex-1 flex-col gap-4 p-4">
  <EmailsFilters {...snippetProps} />
  <EmailsTable {...snippetProps} />
</div>
