...
<script lang="ts">
  ...
  import { normalizeRoute, slugify } from '$lib/utils/casing'
  ...
  // Track if route was manually edited by user
  // Auto-generated routes (like /page-abc123) should allow auto-updates from title
  // Custom routes should be preserved
  const isAutoGeneratedRoute = page.route.startsWith('/page-')
  let routeManuallyEdited = $state(!isAutoGeneratedRoute)
  ...
  function handleTitleChange(e: Event) {
    ...
    const updates: Partial<BuilderPageConfig> = { title: newTitle }

    // Auto-update route if it hasn't been manually edited and title is not empty
    if (!routeManuallyEdited && newTitle.trim()) {
      const sluggedRoute = `/${slugify(newTitle)}`
      updates.route = sluggedRoute
    }

    adminStore.updatePage(page.$id, updates)
  }


  function handleRouteChange(e: Event) {
    const newRoute = normalizeRoute((e.target as HTMLInputElement).value)
    adminStore.updatePage(page.$id, { route: newRoute })
    // Mark route as manually edited
    routeManuallyEdited = true
  }
