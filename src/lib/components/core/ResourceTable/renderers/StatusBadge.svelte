<script lang="ts">
  ...
  export type { StatusVariant } from './StatusBadge.types'
...
<Badge variant="outline" class="px-1.5 text-muted-foreground">
  {#if variant === 'active'}
    <CircleCheckFilled class="size-3.5 fill-green-500 dark:fill-green-400" />
  ...
  {:else if variant === 'alert'}
    <AlertCircleFilled class="size-3.5 fill-amber-500 dark:fill-amber-400" />
