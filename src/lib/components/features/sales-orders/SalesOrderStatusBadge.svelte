<!--
  ...
  @description Badge showing sales order status with a colored state icon.
  Combines the outline Badge with icon indicators for open/approved/rejected states.
...
<script lang="ts">
  ...
  let { state }: { state: SalesOrder['state'] } = $props()
</script>
...
<Badge variant="outline" class="px-1.5">
  ...
  {/if}
  {getSalesOrderStatusLabel(state)}
</Badge>
