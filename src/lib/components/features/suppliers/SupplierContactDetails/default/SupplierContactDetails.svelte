...
<script lang="ts">
  ...
  const detail = useDetailRecord<SupplierContact>({
    getUuid: () => uuid,
    fetch: id =>
      api.safe.get<SupplierContact>(`/legal-entities/${legalEntityId}/suppliers/${supplierId}/contacts/${id}`),
    ...
    getDetailRoute: record =>
      createRoute({ $id: 'supplier-contact-details', params: { uuid: supplierId, cid: record.id } }),
    onFetched: data => {
