<script lang="ts">
  ...
  let { user }: { user: UserResource } = $props()
        ...
        {#snippet child({ props })}
          <Sidebar.MenuButton
            ...
            <Avatar.Root class="size-8 rounded-md">
              ...
              <Avatar.Fallback class="rounded">{getUserInitials(user.name)}</Avatar.Fallback>
            </Avatar.Root>
        ...
        <DropdownMenu.Label class="p-0 font-normal">
          <div class="flex items-center gap-2 px-1 py-1.5 text-start text-sm">
            <Avatar.Root class="size-8 rounded">
              <Avatar.Image src={user.id} alt={user.name} />
              <Avatar.Fallback class="rounded-md">{getUserInitials(user.name)}</Avatar.Fallback>
            </Avatar.Root>
            <div class="grid flex-1 text-start text-sm leading-tight">
              <span class="truncate font-medium">{user.name}</span>
              <span class="truncate text-xs">{user.email}</span>
